...
theme='FF factors'
plotff.plot(subplots=True, layout=(6,1), figsize=(10,15), title=theme)
plt.ylabel('Value Weighted Return')
plt.xlabel('Time intervals from '+datelist[0] + ' to '+datelist[-1])
plt.savefig(datelist[0]+'_to_'+datelist[-1]+'smb.pdf')
            
plt.figure(figsize=(10,15))
((plotff+1).cumprod()-1).plot(subplots=True, layout=(6,1), figsize=(10,15), title=theme)
plt.savefig(datelist[0]+'_to_'+datelist[-1]+'_cumulative_smb.pdf')
###################
#  Saving results  #
###################
ff_factors_merged.to_csv(datelist[0][2:]+'_'+datelist[-1][4:]+"_daily_all_newmathcing_smb.csv")
