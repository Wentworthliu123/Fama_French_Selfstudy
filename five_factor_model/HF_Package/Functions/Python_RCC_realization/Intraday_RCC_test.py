...
for eachday in datelist:
    kd = TAQ.read('DailyLink', date = int(eachday))
...
kd_m_p5m=kd_m.copy().sort_values(by=['date','permno','intratime'])
...
for eachday in datelist:
    ...
    dff = dff.sort_values(by=['date', 'permno', 'intratime'])
    dff['intratime'] =dff['intratime'].str[1:].astype(int)
    dff.reset_index(drop='true')
    all_p5m = pd.concat([all_p5m,dff])
kd_m_p5m=pd.merge(kd_m_p5m, all_p5m, how='left',on=['date','permno','ticker','intratime'])
