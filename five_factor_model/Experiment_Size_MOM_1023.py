...
#ccm5 中的
#['date','permno','shrcd','exchcd','retadj','me','wt','cumretx','ffyear','jdate','permno','ffyear','szport','posbm']

#取出nyse成分，注意原来算过一次cumretx，这里抄wrds mom的计算用的是cumret，我先retadj改算看能得到什么
ccm5=crsp3[(crsp3['wt']>0) & ((crsp3['shrcd']==10) | (crsp3['shrcd']==11))].copy()
ccm5['ret']=ccm5['retadj']
_tmp_crsp = ccm5[['permno','date','ret','me','exchcd']].sort_values(['permno','date'])\
    .set_index('date')
...
ccm1_mom['jdate']=ccm1_mom['date']+MonthEnd(0)
...
vwret=ccm7.groupby(['jdate','szport','momport']).apply(wavg, 'retadj','wt').to_frame().reset_index().rename(columns={0: 'vwret'})
