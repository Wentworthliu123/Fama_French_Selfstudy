...
crspd2 = crspd2[['permno','shrcd','exchcd','retadj','me','wt','jdate','date']]

ccm3=pd.merge(crspd2, everymom[['permno', 'jdate', 'momport','szport','posbm','nonmissport']], how='left', on=['permno','jdate'])
#['permno','date', 'jdate', 'momport','szport','posbm','nonmissport']
# keeping only records that meet the criteria
ccm7=ccm3[(ccm3['wt']>0)& (ccm3['posbm']==1) & (ccm3['nonmissport']==1) & 
...
vwret=ccm7.groupby(['date','szport','momport']).apply(wavg, 'retadj','wt').to_frame().reset_index().rename(columns={0: 'vwret'})
vwret['smport']=vwret['szport']+vwret['momport']

# firm count
vwret_n=ccm7.groupby(['date','szport','momport'])['retadj'].count().reset_index().rename(columns={'retadj':'n_firms'})
vwret_n['smport']=vwret_n['szport']+vwret_n['momport']

# tranpose
ff_factors=vwret.pivot(index='date', columns='smport', values='vwret').reset_index()
ff_nfirms=vwret_n.pivot(index='date', columns='smport', values='n_firms').reset_index()
...
# # output data #
# ###################
writer = pd.ExcelWriter('Daily_MOM_9018.xlsx', engine='xlsxwriter')
