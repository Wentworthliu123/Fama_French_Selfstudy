...
infile='F:\\RA_Fama_French_Factor\\five_factor_model\\SIZE_MOM\\Daily_MOM_9018.xlsx'
...
coefficient = stats.pearsonr(_ffcomp[wfactor], _ffcomp[factor])
print(coefficient)

addtitle = str(round(coefficient[0]*100,2))+"%"
...
plt.ylabel("Return")
plt.title(factortitle)
plt.plot(_ffcomp['date'],_ffcomp[factor],label = 'FF_'+factortitle,color='red')
plt.plot(_ffcomp['date'],_ffcomp[wfactor], label = 'Rep_'+factortitle,color='blue')
plt.legend(loc="best")
...
pp = PdfPages(stitle+"_"+addtitle+".pdf")
pp.savefig(pdf1)
pp.savefig(pdf2)
pp.close()
