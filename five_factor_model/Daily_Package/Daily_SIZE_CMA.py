...
##########################################
# Fama French Factors -- Monthly SMB and CMA
# Oct 20 2019
...
#eg:from 2015-02-04锛坉type: object�?to 2015-02-04(datetime64[ns])
...
ccm1=pd.merge(comp[['gvkey','datadate','be', 'inv', 'count']],ccm,how='left',on=['gvkey'])
ccm1['yearend']=ccm1['datadate']+YearEnd(0)
ccm1['jdate']=ccm1['yearend']+MonthEnd(6)

# set link date bounds
ccm2=ccm1[(ccm1['jdate']>=ccm1['linkdt'])&(ccm1['jdate']<=ccm1['linkenddt'])]
ccm2=ccm2[['gvkey','permno','datadate','yearend', 'jdate','be','inv', 'count']]
...
june=ccm1_jun[['permno','date', 'jdate', 'caport','szport','posbm','nonmissport']]
...
crsp_d = conn.raw_sql("""
                      ...
                      and b.namedt<=a.date
                      and a.date<=b.nameendt
                      where a.date between '07/01/1990' and '06/30/2018'
                      and b.exchcd between 1 and 3
                      """) 
# change variable format to int
...
# merge back with daily records
crspd2 = crspd2[['permno','shrcd','exchcd','retadj','me','wt','ffyear','jdate']]
ccm3=pd.merge(crspd2, 
        june[['permno','ffyear','szport','caport','posbm','nonmissport']], how='left', on=['permno','ffyear'])
...
vwret=ccm4.groupby(['jdate','szport','caport']).apply(wavg, 'retadj','wt').to_frame().reset_index().rename(columns={0: 'vwret'})
vwret['sbport']=vwret['szport']+vwret['caport']

# firm count
vwret_n=ccm4.groupby(['jdate','szport','caport'])['retadj'].count().reset_index().rename(columns={'retadj':'n_firms'})
vwret_n['sbport']=vwret_n['szport']+vwret_n['caport']
...
ff_factors['WB']=(ff_factors['BA']+ff_factors['BM']+ff_factors['BC'])/3
ff_factors['WS']=(ff_factors['SA']+ff_factors['SM']+ff_factors['SC'])/3
...
ff_nfirms['CMA']=ff_nfirms['A']+ff_nfirms['C']

ff_nfirms['B']=ff_nfirms['BC']+ff_nfirms['BM']+ff_nfirms['BA']
ff_nfirms['S']=ff_nfirms['SC']+ff_nfirms['SM']+ff_nfirms['SA']
...
# # output data #
# ###################
writer = pd.ExcelWriter('FF_Model_CMA8018.xlsx', engine='xlsxwriter')
ff_factors.to_excel(writer,sheet_name='ff_factors') 
ff_nfirms.to_excel(writer,sheet_name='ff_nfirms') 
nyse_inv.to_excel(writer,sheet_name='nyse_inv') 
