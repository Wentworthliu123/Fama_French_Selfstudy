...
%crspmerge(s=d,start=01jun2015,end=31dec2017,
...
proc sql; create table crspd2
 as select a.*, b.dlret,
  sum(1,ret)*sum(1,dlret)-1 as retadj "Return adjusted for delisting",
  abs(a.prc)*a.shrout as MEq 'Market Value of Equity'
 from Crsp_d a left join crsp.msedelist(where=(missing(dlret)=0)) b
 on a.permno=b.permno and
    intnx('day',a.date,0,'E')=intnx('day',b.DLSTDT,0,'E')
...
data crspd2a (drop = Meq); set crspd2;
  by date permco Meq;
  retain ME;
...
proc sort data=crspd2a nodupkey; by permno date;run;
...
data crspd3 (keep=permno date retadj weight_port ME exchcd shrcd cumretx)
...
proc sql;
  ...
  select a.*, b.DEC_ME
  from crspd3 (where=(month(date)=6)) as a, decme as b
...
proc export data=myh.ff_factors outfile='wrds_factors_2017.txt'
dbms=tab replace;
run;
